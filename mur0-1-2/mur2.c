...
void comprovar_bloc(int f, int c, int* index)
{
    ...
    if (quin == BLKCHAR || quin == FRNTCHAR) {
        ...
        if (quin == BLKCHAR && *index < npilotes){
            *index = *index + 1;
            /* generar la pilota */
            win_escricar(f_pil[*index], c_pil[*index], '1', INVERS);

            if (pthread_create(&tid[*index],NULL,mou_pilota,(void *)(intptr_t) *index) == 0) {
                n++;
            }
        }
    ...
    do{
        ...
        if ((f_h != f_pil[in]) || (c_h != c_pil[in])) {
            ...
            if (f_h != f_pil[in]) {	/* provar rebot vertical */
                ...
                pthread_mutex_unlock(&mutex); /* tanca semafor */
                if (rv != ' ') {	/* si hi ha alguna cosa */
                    pthread_mutex_lock(&mutex); /* tanca semafor */
                    comprovar_bloc(f_h, c_pil[in], &inAux);
                    pthread_mutex_unlock(&mutex); /* tanca semafor */
            ...
            if (c_h != c_pil[in]) {	/* provar rebot horitzontal */
                ...
                if (rh != ' ') {	/* si hi ha algun obstacle */
                    pthread_mutex_lock(&mutex); /* tanca semafor */
                    comprovar_bloc(f_pil[in], c_h, &inAux);
                    pthread_mutex_unlock(&mutex); /* tanca semafor */
...
int main(int n_args, char *ll_args[])
{
    ...
    i = 0;
    //desde
    if (pthread_create(&tid[n],NULL,mou_paleta, NULL) == 0)
        n++;
